...
export async function fundPoolTransaction(
  ...
  amount: number,
  rewardExtensionInDays: number
): Promise<Transaction> {
  const connection = ConnectionService.getConnection();
  const rewardExtension = rewardExtensionInDays * Constants.secondsInOneDay;
  const rewardsATAPubkey = await findAssociatedTokenAddress(
    funderWallet,
    Pubkeys.rewardsMintPubkey
  );
  const amountToFund = new BN(amount * Constants.decimalPrecision).mul(
    new BN(Constants.toRewardTokenRaw / Constants.decimalPrecision)
  );
  const fundPoolIx = new TransactionInstruction({
    ...
    data: Buffer.from([
      CwarStakingInstructions.FundPool,
      ...amountToFund.toArray('le', 8),
      ...new BN(rewardExtension).toArray('le', 8),
    ]),
  });
